...
function binarySearchIndex(list, target, low=0, high=list.length-1) {
  ...
  let slicePt = Math.floor((high + low) / 2)

  if (target < list[slicePt]) {
    return binarySearchIndex(list, target, low, slicePt)
  } else if (target > list[slicePt]) {
    return binarySearchIndex(list, target, slicePt + 1, high)
  } else {
    return slicePt;
  }
}
